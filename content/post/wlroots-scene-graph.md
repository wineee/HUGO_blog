...
`wlr_scene_tree` 是唯一能有子节点的类型，子节点存储在一个 `wl_list` 中，按**从后到前**（painter's algorithm）的顺序排列。它本身不产生任何像素，只是一个坐标变换和可见性的传递容器。
...
- 处理 `wlr_xdg_surface`、`wlr_layer_surface` 等协议上层的弹出菜单（popup）

---

## 坐标系

scene graph 使用**输出布局坐标**（output-layout coordinates），即全局逻辑坐标空间：
